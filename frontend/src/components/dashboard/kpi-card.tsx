...
export function KPICard({ label, value, helperText, icon: Icon, variant, loading }: KPICardProps) {
  ...
  const helperId = `kpi-${label.toLowerCase().replace(/\s+/g, '-')}-desc`

  return (
    <Card className="border-border/60 hover:border-border transition-colors focus-within:border-ring">
      <CardContent className="p-6 flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <span
            id={`${helperId}-label`}
            className="text-sm font-medium text-muted-foreground tracking-wide uppercase text-pretty"
          >
            {label}
          </span>
          <span
          ...
          >
            <Icon size={16} className={styles.icon} focusable={false} />
          </span>
        </div>
        <p
          className="text-3xl font-semibold tracking-tight text-foreground"
          aria-labelledby={`${helperId}-label`}
